...
function getPixelColor(data: Buffer, width: number, height: number, x: number, y: number, channels: number): number {
   ...
   */
function findIslands(pixels: Pixel[]): Pixel[][] {
    const islands: Pixel[][] = [];
...
function traceContour(islandPixels: Pixel[]): [number, number][] {
    if (islandPixels.length < 3) return [];
    ...
    while (safety++ < MAX_STEPS) {
      ...
      if (!foundNext) break;
      if (cx === startNode.x && cy === startNode.y && contour.length > 2) break;
    }
    return contour;
  }
...
function pointToLineDistance(p: [number, number], a: [number, number], b: [number, number]) {
...
async function processMap() {
    ...
    for (const [colorId, region] of colorMap) {
      if (region.pixels.length < MIN_PIXELS) continue;

      // A. Find ALL Islands
      const islands = findIslands(region.pixels);
    
      // B. Trace ALL Valid Islands
      const validContours: [number, number][][] = [];
      let largestIsland: Pixel[] = [];

      for (const island of islands) {
        if (island.length < MIN_PIXELS) continue; // Skip mostly noise
        if (island.length > largestIsland.length) largestIsland = island;
        ...
        if (simple.length >= 3) validContours.push(simple);
      }
    ...
    paths.forEach(dir => {
      ...
      fs.writeFileSync(path.join(dir, 'provinces.json'), JSON.stringify(territories, null, 2));
      fs.writeFileSync(path.join(dir, 'world-graph.json'), JSON.stringify(graph, null, 2));
    });
