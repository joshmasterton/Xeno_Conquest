...
export function createAIUnits(
  ...
  for (let i = 0; i < count; i++) {
    ...
    // try a few alternatives if path not found
    ...
    const speed = UNIT_BASE_SPEED; // consistent speed for AI units
    units.push({
      id: `ai-${i + 1}`,
      edgeId,
      distanceOnEdge: 0,
...
export function createAIUnitsFromBases(
  ...
  for (const baseId of baseNodeIds) {
    ...
    for (let i = 0; i < perBase && uid <= count; i++, uid++) {
      // choose an initial edge oriented away from base
      ...
      if (!startEdge && fallbackIncoming[0]) {
        ...
        // Use reverse orientation if needed (we synthesized reverse edges)
        startEdge = edges.find(
          (e) => e.sourceNodeId === baseId && e.targetNodeId === inc.sourceNodeId
        ) || inc;
      }
      if (!startEdge) {
        // absolute fallback: any edge
        startEdge = edges[0];
        startNodeId = startEdge.sourceNodeId;
      }

      // plan a random destination via BFS
...
export function updateAIPaths(
  ...
  for (const unit of units) {
    // Skip player-owned units; only AI units get auto-pathfinding
    ...
    if (arrivedAtNode && hasNoPlan) {
      // Pick a new random destination and plan a path from current node
      ...
      if (path && path.length >= 2) {
        ...
        // fallback: continue along any outgoing edge if available
